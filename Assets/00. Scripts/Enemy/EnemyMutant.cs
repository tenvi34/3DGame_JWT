...
{
    // ==해야 하는 거==
    // 애니메이션에서 MoveSpeed로 뮤턴트의 걷기-달리기 애니메이션 전환 기능
    // 순찰상태에서는 걷기, 추격 상태에서는 MoveSpeed 값 변경해서 달리기로 전환
    public class EnemyMutant : LivingEntity, IEnemy
    {
        // 뮤턴트 스탯
        [Header("Mutant Stats")]
        ...
        public float WalkSpeed => mutantWalkSpeed;
        public float RunSpeed => mutantRunSpeed;
        ...
        private void Start()
        {
            ...
            // 적 능력치 설정
            Setup(mutantHealth, mutantDamage, mutantWalkSpeed);
        }
        ...
        public void OnAttackPoint()
        {
            ...
            if (_enemyController.Player != null && IsInAttackRange(_enemyController.Player.transform.position))
            {
                if (_enemyController.Player.TryGetComponent<IDamageable>(out var target))
                {
                    ...
                    target.OnDamage(mutantDamage, hitPoint, hitNormal);
                }
            }
        }
    }
